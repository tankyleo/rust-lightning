...
fn create_nodes_using_cfgs(cfgs: Vec<MessengerCfg>) -> Vec<MessengerNode> {
	...
	for (i, cfg) in cfgs.into_iter().enumerate() {
		let secret_key = cfg
			.secret_override
			.unwrap_or(crate::prelude::secret_key_from_slice(&[(i + 1) as u8; 32]).unwrap());
		let logger = Arc::new(TestLogger::with_id(format!("node {}", i)));
		let seed = [i as u8; 32];
		let entropy_source = Arc::new(TestKeysInterface::new(
			&seed,
			Network::Testnet(bitcoin::network::TestnetVersion::V3),
		));
...
fn spec_test_vector() {
	...
	.iter()
	.map(|secret_hex| {
		crate::prelude::secret_key_from_slice(&<Vec<u8>>::from_hex(secret_hex).unwrap()).unwrap()
	})
	...
	let blinding_key =
		crate::prelude::secret_key_from_slice(&<Vec<u8>>::from_hex(blinding_key_hex).unwrap())
			.unwrap();
