...
/// hash 32 bytes, index 4 bytes, script_sig len 1 bytes, script_sig 0 bytes, sequence 4 bytes
const SEGWIT_TXIN_NON_WITNESS_WEIGHT: u64 = 41 * WITNESS_SCALE_FACTOR as u64;
/// value 8 bytes, var_int 1 byte, p2wsh script len 34 bytes
const P2WSH_TXOUT_WEIGHT: u64 = 43 * WITNESS_SCALE_FACTOR as u64;
...
pub fn htlc_success_input_output_pair_weight(channel_type_features: &ChannelTypeFeatures) -> u64 {
	let witness_weight = if channel_type_features.supports_anchors_zero_fee_htlc_tx() {
		HTLC_SUCCESS_INPUT_KEYED_ANCHOR_WITNESS_WEIGHT
	} else {
		HTLC_SUCCESS_INPUT_P2A_ANCHOR_WITNESS_WEIGHT
	};
	SEGWIT_TXIN_NON_WITNESS_WEIGHT + P2WSH_TXOUT_WEIGHT + witness_weight
}
