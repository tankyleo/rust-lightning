	...
	<SP::Target as SignerProvider>::EcdsaSigner: EcdsaChannelSigner,
{
	...
	pub fn splice_channel(
		...
		let balance_remaining = self.validate_our_funding_contribution_reserve(&self.funding)?;
		...
		let _balance_remaining_msat =
			self.validate_our_funding_contribution_reserve(&dummy_splice_funding)?;
	...
	#[cfg(splicing)]
	fn validate_our_funding_contribution_reserve(
		&self, splice_funding: &FundingScope,
	) -> Result<Amount, APIError> {
		let validate_stats = |stats: &NextCommitmentStats| {
			let (local_balance_incl_fee_msat, _) = stats.get_balances_including_fee_msat();
		...
		self.validate_local_and_remote_stats(splice_funding, validate_stats)
	}
	...
	fn validate_local_and_remote_stats<E>(
		&self, splice_funding: &FundingScope,
		validate_stats: impl Fn(&NextCommitmentStats) -> Result<Amount, E>,
	) -> Result<Amount, E> {
		...
		let on_local_tx_balance_remaining = validate_stats(&splice_local_commitment_stats)?;
		...
		let on_remote_tx_balance_remaining = validate_stats(&splice_remote_commitment_stats)?;
