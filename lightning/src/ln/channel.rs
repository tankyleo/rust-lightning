...
impl FundingScope {
	...
	pub fn get_funding_output(&self) -> Option<TxOut> {
		self.channel_transaction_parameters.get_taproot_output()
	}
	...
	fn to_splice_funding_input(&self) -> SharedOwnedInput {
		...
		let prev_output = self.get_funding_output().unwrap();
...
trait InitialRemoteCommitmentReceiver<SP: SignerProvider> {
	...
	fn check_counterparty_commitment_signature<L: Logger>(
		...
		log_trace!(logger, "Checking {} tx signature {} by key {} against tx {} (sighash {}) with redeemscript {} for channel {}.",
			...
			encode::serialize_hex(&self.funding().get_funding_output().unwrap()), &self.context().channel_id());
...
impl<SP: SignerProvider> ChannelContext<SP> {
	...
	> {
		...
		let commitment_txid = {
			...
			let sighash = bitcoin_tx.get_sighash_default(&funding.get_funding_output().unwrap());
			let session = Session::new(&key_agg_cache, agg_nonce, sighash.as_ref());
			if !session.partial_verify(&key_agg_cache, &msg.signature.partial_signature, &msg.signature.public_nonce, funding.get_counterparty_pubkeys().funding_pubkey) {
				return Err(ChannelError::close(
	...
	fn check_for_funding_tx_confirmed<L: Logger>(
		...
		if funding.funding_tx_confirmation_height == 0 {
			if tx.txid() == funding_txo.txid {
					...
					if txo_idx >= tx.outputs.len() || tx.outputs[txo_idx].script_pubkey != funding.get_funding_output().unwrap().script_pubkey ||
...
impl FundingNegotiationContext {
	...
	fn into_interactive_tx_constructor<SP: SignerProvider, ES: EntropySource>(
		...
		let shared_funding_output = funding.get_funding_output().unwrap();
	...
	SP::EcdsaSigner: EcdsaChannelSigner,
{
	...
	fn build_signed_closing_transaction(
		...
		tx.inputs[0].witness.push(sig.as_byte_array());
	...
	pub fn closing_complete<L: Logger>(
		...
		let session = Session::new(&key_agg_cache, agg_nonce, sighash.as_ref());
		if !session.partial_verify(&key_agg_cache, &counterparty_sig.partial_signature, &counterparty_sig.public_nonce, self.funding.get_counterparty_pubkeys().funding_pubkey) {
			return Err(ChannelError::close(
	...
	pub fn closing_sig<L: Logger>(
		...
		let sighash = closing_tx.trust().get_sighash_default(&self.funding.get_funding_output().unwrap());
		let session = Session::new(&key_agg_cache, agg_nonce, sighash.as_ref());
		if !session.partial_verify(&key_agg_cache, &counterparty_sig_with_nonce.partial_signature, &counterparty_sig_with_nonce.public_nonce, self.funding.get_counterparty_pubkeys().funding_pubkey) {
			return Err(ChannelError::close(
	...
	fn send_commitment_no_state_update_for_funding<L: Logger>(
...

		{
			...
			log_trace!(logger, "Signed remote commitment tx {} (txid {}) with redeemscript {} -> {}",
				...
				&trusted_tx.txid(), encode::serialize_hex(&funding.get_funding_output().unwrap()),
...
mod tests {
	...
	fn test_holder_vs_counterparty_dust_limit() {
		...
		let funding_output = node_a_chan.funding.get_funding_output().unwrap();
		let tx = Transaction { version: Version::ONE, lock_time: LockTime::ZERO, inputs: Vec::new(), outputs: vec![funding_output]};
	...
	fn blinding_point_skimmed_fee_malformed_ser() {
		...
		outbound_chan
			...
			.unwrap();
		let funding_output = outbound_chan.funding.get_funding_output().unwrap();
		let tx = Transaction {
			version: Version::ONE,
