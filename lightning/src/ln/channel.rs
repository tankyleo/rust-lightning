	...
	SP::Target: SignerProvider,
{
		...
		F::Target: FeeEstimator,
	{
		...
		let next_remote_commitment_stats = self.get_next_remote_commitment_stats(funding, Some(HTLCAmountDirection { outbound: false, amount_msat: msg.amount_msat }), false, 0, self.feerate_per_kw, dust_exposure_limiting_feerate);
		...
		let next_local_commitment_stats = self.get_next_local_commitment_stats(funding, Some(HTLCAmountDirection { outbound: false, amount_msat: msg.amount_msat }), false, 0, self.feerate_per_kw, dust_exposure_limiting_feerate);
		if funding.is_outbound() {
			// Check that they won't violate our local required channel reserve by adding this HTLC.
			if next_local_commitment_stats.holder_balance_msat.unwrap() < funding.counterparty_selected_channel_reserve_satoshis.unwrap() * 1000 + next_local_commitment_stats.commit_tx_fee_sat * 1000 {
