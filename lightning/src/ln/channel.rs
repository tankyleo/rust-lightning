...
impl<SP: Deref> ChannelContext<SP> where SP::Target: SignerProvider {
			...
			SP::Target: SignerProvider,
	{
		...
		if stats.remote_balance_before_fee_msat < stats.total_fee_sat * 1000 {
			return Err(ChannelError::close(format!("Funding amount ({} sats) can't even pay fee for initial commitment transaction fee of {} sats.", stats.remote_balance_before_fee_msat / 1000, stats.total_fee_sat)));
		}

		let to_remote_satoshis = stats.remote_balance_before_fee_msat / 1000 - stats.total_fee_sat;
			...
			L::Target: Logger,
	{
		...
		if stats.local_balance_before_fee_msat < stats.total_fee_sat * 1000 {
			return Err(APIError::APIMisuseError{ err: format!("Funding amount ({}) can't even pay fee for initial commitment transaction fee of {}.", stats.local_balance_before_fee_msat / 1000, stats.total_fee_sat) });
		}
	...
	#[inline]
	#[allow(dead_code)]
	// HTLCs to include: those generated by local, those not generated by local, and all ?
	fn build_commitment_stats(&self, funding: &FundingScope, local: bool, generated_by_local: bool, include_all: bool, feerate_per_kw: Option<u32>, buffer_nondust_htlcs: usize) -> CommitmentStats {
		...
		where L::Target: Logger
	{
		...
		debug_assert_eq!(stats, self.build_commitment_stats(funding, local, generated_by_local, false, None, 0));
	...
	<SP::Target as SignerProvider>::EcdsaSigner: EcdsaChannelSigner
{
	...
	pub fn update_add_htlc<F: Deref>(
		...
		let stats = self.context.build_commitment_stats(&self.funding, false, false, false, Some(dust_exposure_limiting_feerate), 0);
		...
		L::Target: Logger
	{
		...
		let stats = self.context.build_commitment_stats(&self.funding, true, true, false, Some(dust_exposure_limiting_feerate), 0);
