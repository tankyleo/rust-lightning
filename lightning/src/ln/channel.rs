...
use crate::ln::chan_utils::{
	...
	ClosingTransaction, commit_tx_fee_sat, HTLCOutput,
};
#[cfg(splicing)]
...
impl<SP: Deref> ChannelContext<SP> where SP::Target: SignerProvider {
		...
		where L::Target: Logger
	{
		...
		let mut included_htlcs: Vec<(HTLCOutput, Option<Box<HTLCSource>>)> = Vec::new();
		...
		macro_rules! add_htlc_output {
			($htlc: expr, $outbound: expr, $source: expr) => {
				let htlc_in_tx = HTLCOutput {
					offered: $outbound == local,
					amount_msat: $htlc.amount_msat,
					cltv_expiry: $htlc.cltv_expiry,
					payment_hash: $htlc.payment_hash,
					id: (u64::from($outbound) << 63) + $htlc.htlc_id,
				};
				included_htlcs.push((htlc_in_tx, $source));
			}
		}
		...
		let nondust_htlcs = included_htlcs.iter().filter_map(|(htlc, _source)| if !htlc.is_dust(feerate_per_kw, broadcaster_dust_limit_sat, self.get_channel_type()) { Some(htlc) } else { None }).cloned().collect();
		...
		let mut htlcs_included: Vec<_> = included_htlcs.iter().map(|(htlc, source)| (HTLCOutputInCommitment::from_htlc_data(htlc.clone(), None), source.clone())).collect();

		let ids_idx = tx.get_id_index_map();

		for (id, index) in ids_idx {
			for (idx, (htlc, _)) in included_htlcs.iter().enumerate() {
				if *id == htlc.id {
					htlcs_included[idx].0.transaction_output_index = Some(*index);
				}
			}
		}
		...
		match &self.context.holder_signer {
			ChannelSignerType::Ecdsa(ecdsa) => {
...

				{
					...
					for (ref htlc_sig, htlc) in htlc_signatures.iter().zip(trusted_tx.nondust_htlcs()) {
