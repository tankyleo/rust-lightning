...
use crate::ln::chan_utils::{commitment_tx_base_weight, COMMITMENT_TX_WEIGHT_PER_HTLC, OFFERED_HTLC_SCRIPT_WEIGHT, htlc_success_tx_weight, htlc_timeout_tx_weight, HTLCOutput};
...
pub fn test_update_fee_that_funder_cannot_afford() {
	...
	let res = {
		...
		let mut nondust_htlcs: Vec<HTLCOutput> = vec![];
...
pub fn test_update_fee_that_saturates_subs() {
	...
	let res = {
		...
		let local_chan_signer = local_chan.get_signer();
		let nondust_htlcs: Vec<HTLCOutput> = vec![];
		let commitment_tx = CommitmentTransaction::new(
...
pub fn test_fee_spike_violation_fails_htlc() {
	...
	let accepted_htlc_info = chan_utils::HTLCOutput {
		offered: false,
		amount_msat: 3460001,
		cltv_expiry: htlc_cltv,
		payment_hash,
