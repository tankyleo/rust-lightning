...
#[test]
fn test_splice_out_initiator_reserve_breach() {
	do_test_splice_out_initiator_reserve_breach(AcceptorBalance::NoBalance, ValidationCase::Passes);
	do_test_splice_out_initiator_reserve_breach(
...
#[cfg(test)]
fn do_test_splice_out_initiator_reserve_breach(
	acceptor_balance: AcceptorBalance, validation_case: ValidationCase,
) {
	let chanmon_cfgs = create_chanmon_cfgs(2);
	let node_cfgs = create_node_cfgs(2, &chanmon_cfgs);
	let mut config = test_default_channel_config();
	// Use a 0FC channel here to focus the test on reserves, everything else removed.
	config.channel_handshake_config.negotiate_anchor_zero_fee_commitments = true;
	...
	// Note as soon as we push 1msat to the acceptor, initiator's balance is 1 full satoshi
	// below the channel value.
	let node_0_balance_sat =
		(channel_value_sat * 1000 - node_1_settled_balance_msat - node_1_htlc_balance_msat) / 1000;
	...
	if matches!(validation_case, ValidationCase::Passes) {
		...
		let splice_out_output_sat =
			node_0_balance_sat - node_0_balance_leftover_amount.to_sat() - estimated_fees;
		...
		// Note in 0FC we've got no fee spike buffer, no commit tx fee, no anchors
		let funding_contribution_sat =
			(node_0_balance_sat - node_0_balance_leftover_amount.to_sat()) as i64 * -1;
