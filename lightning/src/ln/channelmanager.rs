				...
				self.funding_transaction_generated_intern(
					...
					*counterparty_node_id,
					...
					|chan| {
						...
						let expected_spk = chan.context.get_funding_spk(&chan.funding);
								...
								self.pending_events.lock().unwrap().push_back((
									events::Event::SplicePending {
										...
										new_funding_redeem_script: splice_negotiated
											.funding_redeem_script,
										new_funding_spk: splice_negotiated.funding_spk,
									},
									None,
								));
							}
				...
				hash_map::Entry::Occupied(mut chan) => {
					match chan.get_mut().as_unfunded_outbound_v1_mut() {
						Some(unfunded_chan) => {
							...
							(unfunded_chan.funding.get_value_satoshis(), unfunded_chan.context.get_funding_spk(&unfunded_chan.funding), unfunded_chan.context.get_user_id())
						},
						None => {
