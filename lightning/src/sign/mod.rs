...
impl ChannelSigner for InMemorySigner {
	...
	fn validate_holder_commitment(
			...
			holder_tx.nondust_htlcs().iter().zip(holder_tx.counterparty_htlc_sigs.iter())
		{
			...
			let htlc_sighashtype = if channel_parameters
				.channel_type_features
				.supports_anchors_zero_fee_htlc_tx()
				|| channel_parameters.channel_type_features.supports_anchor_zero_fee_commitments()
			{
...
impl EcdsaChannelSigner for InMemorySigner {
	...
	fn create_funding_signed(
		...
		Ok(crate::ln::msgs::FundingSigned { channel_id, signature })
	}
