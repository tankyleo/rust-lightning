...
use crate::ln::chan_utils::{
	...
	MAX_STANDARD_TX_WEIGHT, P2A_ANCHOR_INPUT_WITNESS_WEIGHT, TRUC_CHILD_MAX_WEIGHT,
	TRUC_MAX_WEIGHT,
};
	...
	async fn select_confirmed_utxos_internal(
		...
		let mut selected_utxos;
		{
			...
			let mut eligible_utxos = utxos
				...
				.collect::<Vec<_>>();
			eligible_utxos.sort_unstable_by(|(utxo_a, _), (utxo_b, _)| {
				utxo_a
					.output
					.value
					.cmp(&utxo_b.output.value)
					.then(utxo_b.satisfaction_weight.cmp(&utxo_a.satisfaction_weight))
			});
			...
			for (utxo, fee_to_spend_utxo) in eligible_utxos {
				...
				while selected_utxos_weight + utxo.satisfaction_weight > max_utxo_selection_weight
					&& !selected_utxos.is_empty()
				{
					let (smallest_value_utxo, fee_to_spend_utxo): (Utxo, Amount) =
						selected_utxos.pop_front().unwrap();
					selected_amount -= smallest_value_utxo.output.value;
					total_fees -= fee_to_spend_utxo;
					selected_utxos_weight -=
						BASE_INPUT_WEIGHT + smallest_value_utxo.satisfaction_weight;
				}
