	...
	fn finalize_input<Signer: EcdsaChannelSigner>(&self, bumped_tx: &mut Transaction, i: usize, onchain_handler: &mut OnchainTxHandler<Signer>) -> bool {
		match self {
			...
			PackageSolvingData::RevokedHTLCOutput(ref outp) => {
				//TODO: should we panic on signer failure ?
				if let Ok(tx) = onchain_handler.signer.punish_htlc_output(&bumped_tx, i, outp.amount, &outp.per_commitment_key, &onchain_handler.secp_ctx, &outp.per_commitment_point, &outp.htlc) {
					*bumped_tx = tx;
				} else { return false; }
			},
			PackageSolvingData::CounterpartyOfferedHTLCOutput(ref outp) => {
				if let Ok(tx) = onchain_handler.signer.sweep_counterparty_offered_htlc_output(&bumped_tx, i, outp.htlc.amount_msat / 1000, &onchain_handler.secp_ctx, &outp.per_commitment_point, &outp.htlc, &outp.preimage) {
					*bumped_tx = tx;
				}
			},
			PackageSolvingData::CounterpartyReceivedHTLCOutput(ref outp) => {
				if let Ok(tx) = onchain_handler.signer.sweep_counterparty_received_htlc_output(&bumped_tx, i, outp.htlc.amount_msat / 1000, &onchain_handler.secp_ctx, &outp.per_commitment_point, &outp.htlc) {
					*bumped_tx = tx;
				}
			},
			_ => { panic!("API Error!"); }
