...
use crate::ln::chan_utils::{self,CommitmentTransaction, CounterpartyCommitmentSecrets, HTLCOutputInCommitment, HTLCClaim, ChannelTransactionParameters, HolderCommitmentTransaction, TxCreationKeys, HTLCData};
...
pub(crate) enum ChannelMonitorUpdateStep {
	...
	LatestCounterpartyCommitmentTX {
		...
		htlc_outputs: Vec<(HTLCData, Option<Box<HTLCSource>>)>,
...
struct FundingScope {
	...
	// the indices
	counterparty_htlc_indices: HashMap<Txid, Vec<(bool, u64, u32)>>,
...
pub(crate) struct ChannelMonitorImpl<Signer: EcdsaChannelSigner> {
	...
	counterparty_htlc_data: HashMap<u64, Vec<(HTLCData, Option<Box<HTLCSource>>)>>,
...
impl<Signer: EcdsaChannelSigner> Writeable for ChannelMonitorImpl<Signer> {
	fn write<W: Writer>(&self, writer: &mut W) -> Result<(), Error> {
		...
		macro_rules! serialize_htlc_in_commitment_v2 {
			($htlc_output: expr) => {
				writer.write_all(&[$htlc_output.offered as u8; 1])?;
				writer.write_all(&$htlc_output.amount_msat.to_be_bytes())?;
				writer.write_all(&$htlc_output.cltv_expiry.to_be_bytes())?;
		...
		for (ref commitment_number, ref htlc_infos) in self.counterparty_htlc_data.iter() {
			writer.write_all(&commitment_number.to_be_bytes())?;
			writer.write_all(&(htlc_infos.len() as u64).to_be_bytes())?;
			for &(ref htlc_output, ref htlc_source) in htlc_infos.iter() {
				serialize_htlc_in_commitment_v2!(htlc_output);
				htlc_source.as_ref().map(|b| b.as_ref()).write(writer)?;
			}
		}

		writer.write_all(&(self.funding.counterparty_htlc_indices.len() as u64).to_be_bytes())?;
		for (ref txid, ref htlc_indices) in self.funding.counterparty_htlc_indices.iter() {
			writer.write_all(&txid[..])?;
...
impl<Signer: EcdsaChannelSigner> ChannelMonitor<Signer> {
	...
	pub(crate) fn new(
		...
		Self::from_impl(ChannelMonitorImpl {
			funding: FundingScope {
				...
				counterparty_htlc_indices: new_hash_map(),
	...
	#[cfg(test)]
	fn legacy_provide_latest_counterparty_commitment_tx<L: Deref>(
		&self,
		txid: Txid,
		htlc_outputs: Vec<(HTLCOutputInCommitment, Option<Box<HTLCSource>>)>,
		commitment_number: u64,
		...
		let mut inner = self.inner.lock().unwrap();
		let logger = WithChannelMonitor::from_impl(logger, &*inner, None);
		inner.legacy_provide_latest_counterparty_commitment_tx(
...
impl<Signer: EcdsaChannelSigner> ChannelMonitorImpl<Signer> {
	...
	fn get_counterparty_nondust_htlcs(&self, funding: &FundingScope, txid: &Txid, commitment_number: Option<u64>) -> Option<Vec<(HTLCOutputInCommitment, Option<Box<HTLCSource>>)>> {
		if let Some(htlc_id_indices) = funding.counterparty_htlc_indices.get(txid) {
			let commitment_number = commitment_number.as_ref().or_else(|| self.counterparty_commitment_txn_on_chain.get(txid)).unwrap();
			let counterparty_htlc_data = self.counterparty_htlc_data.get(commitment_number).unwrap();
			...
			self.counterparty_claimable_outpoints.get(txid).cloned().map(|mut nondust_htlcs| {
				nondust_htlcs.retain(|(htlc, _source)| htlc.transaction_output_index.is_some());
				nondust_htlcs
			})
		}
	}
...
impl<Signer: EcdsaChannelSigner> ChannelMonitor<Signer> {
	...
	pub fn get_claimable_balances(&self) -> Vec<Balance> {
		...
		if let Some(txid) = confirmed_txid {
			...
			if let Some(populated_htlcs) = us.get_counterparty_nondust_htlcs(&us.funding, &txid, None) {
	...
	pub(crate) fn get_all_current_outbound_htlcs(&self) -> HashMap<HTLCSource, (HTLCData, Option<PaymentPreimage>)> {
	...
	pub(crate) fn get_pending_or_resolved_outbound_htlcs(&self) -> HashMap<HTLCSource, (HTLCData, Option<PaymentPreimage>)> {
		let us = self.inner.lock().unwrap();
				...
				for (htlc, source) in $htlc_iter {
					if us.htlcs_resolved_on_chain.iter().any(|v| v.commitment_tx_output_idx == htlc.transaction_output_index) {
						...
						if !htlc_update_confd || counterparty_resolved_preimage_opt.is_some() {
							res.insert(source.clone(), (htlc.clone().into(), counterparty_resolved_preimage_opt));
						}
					}
				}
			}
		...
		if Some(txid) == us.funding.current_counterparty_commitment_txid || Some(txid) == us.funding.prev_counterparty_commitment_txid {
			walk_htlcs!(false, us.get_counterparty_nondust_htlcs(&us.funding, &txid, None).unwrap().iter().filter_map(|(a, b)| {
...
macro_rules! fail_unbroadcast_htlcs {
	($self: expr, $commitment_tx_type: expr, $commitment_txid_confirmed: expr, $commitment_tx_confirmed: expr,
	 $commitment_tx_conf_height: expr, $commitment_tx_conf_hash: expr, $confirmed_htlcs_list: expr, $logger: expr) => { {
		...
		check_htlc_fails!("current", $self.get_counterparty_htlc_data(true));
		check_htlc_fails!("previous", $self.get_counterparty_htlc_data(false));
	} }
}
...
impl<Signer: EcdsaChannelSigner> ChannelMonitorImpl<Signer> {
	...
	fn provide_secret(&mut self, idx: u64, secret: [u8; 32]) -> Result<(), &'static str> {
		...
		if let Some(prev_txid) = self.funding.prev_counterparty_commitment_txid.take() {
			if self.funding.current_counterparty_commitment_txid.unwrap() != prev_txid {
				...
				let cur_claimables: Vec<(HTLCData, Option<Box<HTLCSource>>)> = self.counterparty_claimable_outpoints.get(
					&self.funding.current_counterparty_commitment_txid.unwrap()
				).map(|data| data.iter().map(|(htlc, source)| (HTLCData::from(htlc.clone()), source.clone())).collect()).unwrap_or_else(||
					self.counterparty_htlc_data.get(&number).unwrap().clone()
				);
				let prev_claimables = self.counterparty_claimable_outpoints.get(&prev_txid)
				.map(|data| data.iter().map(|(htlc, source)| (HTLCData::from(htlc.clone()), source.clone())).collect())
				.unwrap_or_else(||
					self.counterparty_htlc_data.get(&(number + 1)).unwrap().clone()
				);
				...
				for &mut (_, ref mut source_opt) in self.counterparty_htlc_data.get_mut(&(number + 1)).unwrap_or(&mut Vec::new()) {
					*source_opt = None;
				}
			} else {
	...
	fn provide_latest_counterparty_commitment_tx<L: Deref>(
		...
		// TODO: clean this up
		self.counterparty_htlc_data.insert(tx.commitment_number(), htlc_outputs);

		let mut htlc_indices = Vec::new();
		tx.htlcs().iter().for_each(|htlc|
			if let Some(htlc_idx) = htlc.transaction_output_index { htlc_indices.push((htlc.offered, htlc.htlc_id, htlc_idx)) });

		log_trace!(logger, "Tracking new counterparty commitment transaction with txid {} at commitment number {} with {} HTLC outputs", tx.trust().txid(), tx.commitment_number(), tx.htlcs().len());
		self.funding.prev_counterparty_commitment_txid = self.funding.current_counterparty_commitment_txid.take();
		self.funding.current_counterparty_commitment_txid = Some(tx.trust().txid());
		self.funding.counterparty_htlc_indices.insert(tx.trust().txid(), htlc_indices);
		self.current_counterparty_commitment_number = tx.commitment_number();
		//TODO: Merge this into the other per-counterparty-transaction output storage stuff
		match self.their_cur_per_commitment_points {
			Some(old_points) => {
				if old_points.0 == tx.commitment_number() + 1 {
					self.their_cur_per_commitment_points = Some((old_points.0, old_points.1, Some(tx.per_commitment_point())));
				} else if old_points.0 == tx.commitment_number() + 2 {
					if let Some(old_second_point) = old_points.2 {
						self.their_cur_per_commitment_points = Some((old_points.0 - 1, old_second_point, Some(tx.per_commitment_point())));
					} else {
						self.their_cur_per_commitment_points = Some((tx.commitment_number(), tx.per_commitment_point(), None));
					}
				} else {
					self.their_cur_per_commitment_points = Some((tx.commitment_number(), tx.per_commitment_point(), None));
				}
		...
		if let Some(txid) = self.funding.current_counterparty_commitment_txid {
			if txid == confirmed_spend_txid {
				if let Some(commitment_number) = self.counterparty_commitment_txn_on_chain.get(&txid) {
					claim_htlcs!(*commitment_number, txid, self.get_counterparty_nondust_htlcs(&self.funding, &txid, None).as_ref());
				} else {
					debug_assert!(false);
		...
		for update in updates.updates.iter() {
			match update {
				...
				ChannelMonitorUpdateStep::LatestCounterpartyCommitmentTXInfo { commitment_txid, htlc_outputs, commitment_number, their_per_commitment_point, .. } => {
					...
					self.legacy_provide_latest_counterparty_commitment_tx(*commitment_txid, htlc_outputs.clone(), *commitment_number, *their_per_commitment_point, logger)
				},
	...
	where L::Target: Logger {
		...
		let per_commitment_option = self.get_counterparty_nondust_htlcs(&self.funding, &commitment_txid, Some(commitment_number));
	...
	pub fn cancel_prev_commitment_claims<L: Deref>(
		...
		for (counterparty_commitment_txid, commitment_number) in &self.counterparty_commitment_txn_on_chain {
			...
			for (htlc, _) in self.get_counterparty_nondust_htlcs(&self.funding, counterparty_commitment_txid, Some(*commitment_number)).unwrap_or(vec![]) {
	...
	fn is_resolving_htlc_output<L: Deref>(
		...
		'outer_loop: for input in &tx.input {
			...
			if let Some(ref htlc_outputs) = self.get_counterparty_nondust_htlcs(&self.funding, &input.previous_output.txid, None) {
...
impl<'a, 'b, ES: EntropySource, SP: SignerProvider> ReadableArgs<(&'a ES, &'b SP)>
	...
	fn read<R: io::Read>(reader: &mut R, args: (&'a ES, &'b SP)) -> Result<Self, DecodeError> {
		...
		macro_rules! read_htlc_in_commitment_v2 {
			() => {
				{
					let offered: bool = Readable::read(reader)?;
					let amount_msat: u64 = Readable::read(reader)?;
					...
					HTLCData {
						offered, amount_msat, cltv_expiry, payment_hash, htlc_id,
					}
				}
			}
		}
		...
		for _ in 0..counterparty_htlc_data_len {
			let commitment_number: u64 = Readable::read(reader)?;
			let htlcs_count: u64 = Readable::read(reader)?;
			let mut htlcs = Vec::with_capacity(cmp::min(htlcs_count as usize, MAX_ALLOC_SIZE / 32));
			for _ in 0..htlcs_count {
				htlcs.push((read_htlc_in_commitment_v2!(), <Option<HTLCSource> as Readable>::read(reader)?.map(|o: HTLCSource| Box::new(o))));
			}
			if counterparty_htlc_data.insert(commitment_number, htlcs).is_some() {
				return Err(DecodeError::InvalidValue);
			}
		}

		let counterparty_htlc_indices_len: u64 = Readable::read(reader)?;
		let mut counterparty_htlc_indices = hash_map_with_capacity(cmp::min(counterparty_htlc_indices_len as usize, MAX_ALLOC_SIZE / 64));
		for _ in 0..counterparty_htlc_indices_len {
			let txid: Txid = Readable::read(reader)?;
			let htlcs_count: u64 = Readable::read(reader)?;
			let mut htlcs = Vec::with_capacity(cmp::min(htlcs_count as usize, MAX_ALLOC_SIZE / 32));
			for _ in 0..htlcs_count {
			...
			if counterparty_htlc_indices.insert(txid, htlcs).is_some() {
				return Err(DecodeError::InvalidValue);
			}
		}
