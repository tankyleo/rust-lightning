...
use crate::ln::chan_utils::{self,CommitmentTransaction, CounterpartyCommitmentSecrets, HTLCOutputInCommitment, HTLCClaim, ChannelTransactionParameters, HolderCommitmentTransaction, HTLCOutput};
...
impl<Signer: EcdsaChannelSigner> ChannelMonitorImpl<Signer> {
	...
	fn build_counterparty_commitment_tx(
		...
		nondust_htlcs: Vec<(HTLCOutputInCommitment, Option<Box<HTLCSource>>)>
	) -> CommitmentTransaction {
		let channel_parameters = &self.funding.channel_parameters.as_counterparty_broadcastable();
		let nondust_htlcs: Vec<HTLCOutput> = nondust_htlcs.into_iter().map(|(htlc, _)| HTLCOutput::from_output(htlc.clone(), 0)).collect();
...
mod tests {
	...
	use crate::ln::chan_utils::{self,HTLCOutputInCommitment, ChannelPublicKeys, ChannelTransactionParameters, HolderCommitmentTransaction, CounterpartyChannelTransactionParameters, HTLCOutput};
	...
	fn test_prune_preimages() {
		...
		macro_rules! preimages_slice_to_htlc_outputs {
			($preimages_slice: expr) => {
				preimages_slice_to_htlcs!($preimages_slice).into_iter().map(|htlc| (HTLCOutputInCommitment::from_htlc_data(htlc, None), None)).collect()
			}
		}
		...
		let nondust_htlcs = dummy_commitment_tx.nondust_htlcs().clone();

		monitor.provide_latest_holder_commitment_tx(dummy_commitment_tx.clone(),
			nondust_htlcs.into_iter().map(|htlc| (htlc, Some(dummy_sig), None)).collect());
